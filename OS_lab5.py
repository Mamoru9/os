...
#Функция считающая штрафное отношение 
def penalty_against (obj):
    return (obj.time_w8+obj.time)/obj.time
...
if flag == 0:
    ...
    flag = 0
    max_pen = penalty_against(first)
    if max_pen < penalty_against(secound):
        max_pen = penalty_against(secound)
        flag = 1
    if max_pen < penalty_against(fourth):
        max_pen = penalty_against(fourth)
